...
public class GeoCoderDemo extends Activity implements
	...
	private LatLng locData = new LatLng(0,0);//存储位置搜索结果
	...
	private void topbar_event() {
		...
		topbar.setOnTopbarClickListener(new TopBar.topbarClickListener() {
			...
			public void rightClick() throws UnsupportedEncodingException {
				...
				mIntent.putExtra("address", reserve_address);
				// 设置结果，并进行传送
				GeoCoderDemo.this.setResult(1, mIntent);
				finish();
			}
		});
	}
	...
	public void onGetGeoCodeResult(GeoCodeResult result) {
		...
		locData = result.getLocation();
		// 反Geo搜索
		mSearch.reverseGeoCode(new ReverseGeoCodeOption().location(result.getLocation()));
	}
	...
	public void onGetReverseGeoCodeResult(ReverseGeoCodeResult result) {
		...
		reserve_address = result.getAddress();
	}
