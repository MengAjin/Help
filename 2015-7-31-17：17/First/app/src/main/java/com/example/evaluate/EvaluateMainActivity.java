			...
			temp = ea.GetAccount(obj, mcontext, new EvaluateAccounts.getAccount() {
				...
				public void onSuccess1(List<Map<String, Object>> as, long[] a) {
					if(as != null) {
						...
						adapter = new mymyAdapter(EvaluateMainActivity.this, listItemContainer);
						listview.setAdapter(adapter);
						listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
							@Override
							public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
								Intent intent = new Intent(EvaluateMainActivity.this, PersonalEvaluationActivity.class);
								Long[] account = new Long[10];
								int p = 0;
								p = parent.getPositionForView(view);
								account[0] = accounts[p];
								Toast.makeText(EvaluateMainActivity.this, account[0].toString(), Toast.LENGTH_SHORT).show();
								Log.d("TAG", "The click item -> " + account[0]);
