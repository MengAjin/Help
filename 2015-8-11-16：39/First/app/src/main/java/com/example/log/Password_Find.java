            ...
            if (result == SMSSDK.RESULT_COMPLETE) {
                if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) {//提交验证码成功
                    ...
                    JsonRequest<JSONObject> jsonRequest = new JsonObjectRequest(Request.Method.POST,
                            "http://120.24.208.130:1503/account/login", param1,
                                    ...
                                    if (!result.toString().contains("status")) {
                                        ...
                                        getAllParams.getList(url, param2, new GetAllParams.VolleyJsonCallback() {
                                            ...
                                            public void onSuccess(JSONObject result) {
                                                try {
                                                    Log.d("result-find-pwd", result.toString());
                                                    if (result.getInt("status") == 200) {
                                                        Toast.makeText(getApplicationContext(), "修改成功", Toast.LENGTH_SHORT).show();
                                                        finish();
                                                    } else{
                                                        Toast.makeText(getApplicationContext(), "修改失败500", Toast.LENGTH_SHORT).show();
                                        ...
                                        Toast.makeText(getApplicationContext(), "该账号还未注册", Toast.LENGTH_SHORT).show();
                                    }
                                }
